...
@Composable
fun CustomersScreen(viewModel: SublimationViewModel, onNavigateToAddCustomer: () -> Unit, onNavigateToEditCustomer: (Long) -> Unit) {
            ...
            if (customers.isEmpty()) {
                ...
                LazyColumn(modifier = Modifier.fillMaxSize()) {
                    items(customers) { customer ->
                        CustomerItem(customer = customer, onClick = { onNavigateToEditCustomer(customer.id) })
                    }
                }
            }
        }
